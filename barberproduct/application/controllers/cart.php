            ...
            if ($value['cartid'] == $id) {
                if ($str == 'sum') {
                    ...
                    if ($value['qty'] + 1 > $count) {
                        $this->session->set_flashdata('message', 'Number đã hết');
                    } else {
                        $data = array();
                        $data['rowid'] = $key;
        ...
        if ($id > 0) {
            foreach ($carts as $key => $value) {
                if ($value['cartid'] == $id) {
                    ...
                    $data['qty'] = 0;
                    $this->cart->update($data);
                    $this->session->set_flashdata('message', 'Delete sản phẩm thành công');
                    redirect(base_url('cart'));
                }
            }
        } else {
            $this->cart->destroy();
            $this->session->set_flashdata('message', 'Delete giỏ hàng thành công');
            redirect(base_url('cart'));
        }
    }
